...
def main():
    ...
    #filename = "1.14.11.40"
    filename = "1.14.13.46"
    palm_list = raw_interpret2D(filename, 'wrist')
    if len(palm_list) == 0:
    ...
    for val in palm_list:
        ...
        if x_3D != (np.inf or -np.inf) and abs(x_3D)<5:
        ...
        arm_coords, depth = convert_to_arm_coords(x_normalised,y_normalised,joint_depth, True)
