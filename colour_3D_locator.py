...
def click_event(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_LBUTTONDOWN: 
        ...
        #Convert 3D position to arm coordinates
        arm_coords = convert_to_arm_coords(x_3D,y_3D,z_3D)
                
        print("clicked pos is", arm_coords)

def locator_3D():
    """Function which displays a window with the colour footage, in which you can click each pixel to get its 3D position in arm coordinates"""
