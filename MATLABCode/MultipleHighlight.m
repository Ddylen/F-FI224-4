...
% I have significnatly modified this file, based on one originally written by Simon Hauser, so that it can highligh multiple
% subsections of a recipe trajectory, to make nice figures
%% Tabula rasa
%clear all
clc
close all
...
sec = [22,23,1,0] %move spatula to hotplate
plot(data(key_times_list(sec(1)+1)-sec(3):key_times_list(sec(2)+1)-sec(4),1),data(key_times_list(sec(1)+1)-sec(3):key_times_list(sec(2)+1)-sec(4),2), 'Color' ,'#00FF00', 'LineWidth',1.5)
...
sec = [24,26,0,0] %return spatula
plot(data(key_times_list(sec(1)+1)-sec(3):key_times_list(sec(2)+1)-sec(4),1),data(key_times_list(sec(1)+1)-sec(3):key_times_list(sec(2)+1)-sec(4),2), 'Color' ,'#FF00FF', 'LineWidth',1.5)
sec = [26,27,0,10] %idle
...
legendtext = {'Overall Trajectory','Pick up spatula', 'Move spatula to hotplate', 'Flip pancake', 'Return spatula to holder', 'Idle as pancake cooks', 'Pick up spatula', 'Move spatula to hotplate', 'remove pancake'}
...
dotpoint1 = key_times_list(23+1);
dotpoint2 = key_times_list(30+1);

plot(data(dotpoint1,1),data(dotpoint1,2),'r.','MarkerSize',20, 'DisplayName', 'T\_Cook\_2 timer start')
plot(data(dotpoint2,1),data(dotpoint2,2),'k.','MarkerSize',20, 'DisplayName', 'T\_Cook\_2 timer finish')
