%Code to highligh T_Stir portion of teh trajectory, based on modifying coed
%by Simon Hauser
...
sec = [9,10,0,0]  %flip pancake
%end of this is time start
%plot(data(key_times_list(sec(1)+1)-sec(3):key_times_list(sec(2)+1)-sec(4),1),data(key_times_list(sec(1)+1)-sec(3):key_times_list(sec(2)+1)-sec(4),2), 'Color' ,'#0000FF', 'LineWidth',1.5)
%sec = [3,13,0,0] %return spatula
...
legendtext = {'Pick up whisk', 'T\_Stir', 'Return whisk'}
...
dotpoint1 = key_times_list(9+1);
dotpoint2 = key_times_list(10+1);

plot(data(dotpoint1,1),data(dotpoint1,2),'b.','MarkerSize',25, 'DisplayName', 'T\_Stir timer start')
plot(data(dotpoint2,1),data(dotpoint2,2),'r.','MarkerSize',25, 'DisplayName', 'T\_Stir timer finish')
