# -*- coding: utf-8 -*-
"""
Created on Wed Oct 30 14:20:42 2019

Follow me version that got corrupted (maybe i copied another file into it/)
...
sys.path.insert(1,r'C:\Users\birl\Documents\openpose')
...
from realtimehandreader import interpret2D
...
def main():
    ...
    frame_rate = 0.2
    palm_list, closed_list = interpret2D()
    normalised_pos_list = []
    ...
    for val in palm_list:
        if (val[0] > cam_x_max) or (val[0] < cam_x_min) or (val[1] < cam_y_min) or (val[1] > cam_y_max):
            ...
            normalised_pos_list.append(x_normalised,y_normalised, closed_list[palm_list.index(val)])#store normalised position + hand open/closed
    ...
    burt.movej([np.radians(85), np.radians(-77), np.radins(143), np.radians(214), np.radians(-180), np.radians(61)], min_time = 5) #move to first position slowly
