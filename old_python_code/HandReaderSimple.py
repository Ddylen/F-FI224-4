...
Looks like an old version of follow me?
...
def main():
    ...
    burt.movej([np.radians(84), np.radians(-110), np.radins(129), np.radians(245), np.radians(-90), np.radians(45)], min_time = 5) #move to first position slowly
    ...
    for val in normalised_pos_list:
        ...
        #time.sleep(frame_rate)
        burt.movel([move_to_x, move_to_y, 0.2, np.radians(245), np.radians(-90), np.radians(45)], min_time = 3)
        #burt.servoj(burt.get_inverse_kin([move_to_x, move_to_y, 0.4, np.pi/2, 0, 0]), control_time = time_min,lookahead_time=0.008,gain=300)
        #burt.servoj([move_to_x, move_to_y, 0.2, np.radians(245), np.radians(-90), np.radians(45)])
        if val[2] ==1:
            print("hand closed")
        #HAND CLOSE LINE
